...
This directory holds files that are shared between the browserProcessor and nodejsProcessor. Currently this is limited to the Task schema.
# Task Schema
...
* privacy - a copy of the Task object's structure with boolean types that indicate if the property should be sent from the nodejsProcessor to the browserProcessor. For example, this could avoid sending prompts in config data to the web browser.
